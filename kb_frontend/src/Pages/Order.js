...
function Order() {
  ...
  const confirmOrder = () => {
    const selectedAddressInput = document.querySelector(
      'input[name="user_add"]:checked',
    );
    const selectedPaymentInput = document.querySelector(
      '#prc input[type="radio"]:checked',
    );

    if (!selectedAddressInput || !selectedPaymentInput) {
      alert(
        "Please select both delivery address and payment method before confirming the order.",
      );
      return;
    }
    ...
    <div id="Order">
      {!confirmBox && (
        <div id="order-page">
            ...
            <div id="table">
              <table>
                <tbody>
                  ...
                  <tr>
                    <td>Delivery Fee:</td>
                    <td className="result" id={`${tooltip === 0}`}>
                      ₹ {99}.00
                    </td>
                  </tr>
                  <tr>
          ...
          <div className="dlv-address box">
            ...
            <form autoComplete="off" id="optn" onSubmit={dlvAddSave}>
              {address &&
                address.map((add) => {
                  return (
                    <div key={add._id} id={add._id} className="add-optn">
                      <input
                        type="radio"
                        id="user_add"
                        name="user_add"
                        value={`${add.name}, ${add.addtype}, ${add.phone}, ${add.locality}, ${add.area}, ${add.city}, ${add.state}, ${add.pincode} `}
                      />
                      <label htmlFor="user_add">
                        <span className="name">{add.name}</span>{" "}
                        <span className="addtype">{add.addtype}</span>{" "}
                        <span className="no">{add.phone}</span>
                        <div className="adrs">
                          {add.locality}, {add.area}, {add.city}, {add.state} -{" "}
                          {add.pincode}
                        </div>
                      </label>
                      <button
                        className="material-icons-outlined dlt"
                        onClick={deleteAdd}
                      >
                        delete
                      </button>
          ...
          <button className="cnfrm-btn" onClick={confirmOrder}>
            CONFIRM ORDER
          </button>
        </div>
      )}
