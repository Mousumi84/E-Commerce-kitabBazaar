...
function Book() {
  ...
  const clickBook = (e) => {
    ...
    if (
      (e.target.className === "material-icons-outlined like" ||
        e.target.className === "material-icons-outlined notlike") &&
      e.target.id === `love-${bookId}`
    ) {
      ...
      e.target.className === "add_cart" &&
      e.target.id === `cart-${bookId}`
    ) {
      ...
      <div id="book">
        <div id="book-card">
          <div className="image-box">
            <img src={bookdetails?.Coverimg} />
          </div>
          <div className="book-dtl">
            <div className="ttl">
              <span>{bookdetails?.Title}</span> || Paperback
            </div>
            <div className="athr">
              By: <span>{bookdetails?.Author}</span> || Publisher: Penguin
              Publishers India
            </div>
            <div className="cat">
            ...
            <div className="btn">
              ...
              {!like ? (
                <button onClick={likeBook} className="add-wshlt">
                  <FontAwesomeIcon icon={faHeart} size="xl" />
                  SAVE TO WISHLIST{" "}
                </button>
              ) : (
                <button onClick={likeBook} className="rmv-wshlt">
          ...
          <div className="card">
            ...
            <div className="b1">
              <FontAwesomeIcon
                ...
                style={{ color: "#028500" }}
              />{" "}
              <span className="bold">Special Price</span> Get extra ₹20 off on
              20 item(s) (price inclusive of cashback/coupon)
            </div>
          </div>
          <div className="card">
            <div className="h1">Available offers</div>
            <div className="b1">
              <FontAwesomeIcon icon={faTag} style={{ color: "#028500" }} />{" "}
              <span className="bold">Bank offer</span> 5% Unlimited Cashback on
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="book_dtl">
                        ...
                        {isCart || (
                          <button className="add_cart" id={`cart-${book._id}`}>
                        ...
                        {isLogin && (
                          <span
                            className={`material-icons-outlined ${isLiked ? "like" : "notlike"}`}
                            id={`love-${book._id}`}
                          >
                            favorite
                          </span>
